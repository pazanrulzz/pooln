...
- **Pointing local dev at a real AWS table instead of DynamoDB Local**: `pnpm --filter api dev` and `pnpm --filter api test` always force `DYNAMODB_ENDPOINT` to `http://localhost:8000` themselves, regardless of what's in `.env` — so they can never accidentally read/write real data. To use a real table instead, put real `AWS_REGION`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`DYNAMODB_TABLE_NAME` in `.env`, leave `DYNAMODB_ENDPOINT` unset there, and run `pnpm --filter api dev:cloud` (skips local table creation — the real table is provisioned by hand, not by `ensure-table.ts`). Every request then reads/writes the real table, including real DynamoDB cost per request.
